...
class CustomDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "fit":
            ...
            train_scene_count = len(self._train_set)
            logger.info(f"▶▶ 训练集一共加载了 {train_scene_count} 个场景（scenarios）")
            ...
            val_scene_count = len(self._val_set)
            logger.info(f"▶▶ 验证集一共加载了 {val_scene_count} 个场景")
            ...
            test_scene_count = len(self._test_set)
            logger.info(f"▶▶ 测试集一共加载了 {test_scene_count} 个场景")
