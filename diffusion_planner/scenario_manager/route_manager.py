...
class RouteManager:
    ...
    def load_route(self, ego_state: EgoState, process=True) -> None:
        ...
        """
        if process:
            # print(f"[load_route] process")
            # print(f"[load_route] _origin_route_roadblock_ids：{self._origin_route_roadblock_ids}")
            updated_route_roadblock_ids = route_roadblock_correction(
                ego_state, self._map_api, self._origin_route_roadblock_ids
            )
            # print(f"[load_route] 从当前位置：{ego_state.rear_axle.point}查询到route_roadblock_ids：{updated_route_roadblock_ids}")
        else:
            updated_route_roadblock_ids = self._origin_route_roadblock_ids
            # print(f"[load_route] _origin_route_roadblock_ids：{updated_route_roadblock_ids}")
        ...
        # print(f"route_roadblock_id:{updated_route_roadblock_ids}")

        for id_ in updated_route_roadblock_ids:
            block = self._map_api.get_map_object(id_, SemanticMapLayer.ROADBLOCK)
            # if block:
                # print(f"[load_route] block{block.id} {block}")
            ...
            for lane in block.interior_edges:
                # print(f"[load_route] interior_edges_lane {lane.id}")
    ...
    def _get_candidate_starting_lane(self, ego_state: EgoState):
        # print(f"map_name:{self._map_api.map_name}")
        lanes = self._map_api.get_proximal_map_objects(
            ego_state.center.point,
            3,
            [SemanticMapLayer.LANE, SemanticMapLayer.LANE_CONNECTOR],
        )
        # print(f"当前位置：{ego_state.center.point}")
        # for lane in lanes[SemanticMapLayer.LANE]:
            # print(f"    车道{lane.id},在 LANE 层")
        # for lane in lanes[SemanticMapLayer.LANE_CONNECTOR]:
            # print(f"    车道{lane.id},在 LANE_CONNECTOR 层")
        lanes = lanes[SemanticMapLayer.LANE_CONNECTOR] + lanes[SemanticMapLayer.LANE]
        # print(f"筛选前起点候选车道数量：{len(lanes)}")
        # for lane in lanes:
        #     print(f"    车道{lane.id},在{lane.get_roadblock_id()}道路块")
        lanes = [
            lane
            for lane in lanes
        ...
        for lane in lanes:
            for next_lane in lane.outgoing_edges:
                # print(f"{lane.id} next_lane:{next_lane.id}")
                if next_lane.id in keep_ids:
                    keep_ids.remove(next_lane.id)
        # print(f"_route_lane_dict:{self._route_lane_dict}")
